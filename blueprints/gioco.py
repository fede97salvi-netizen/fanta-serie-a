...
from services.game_logic import (
    calcola_punti_pronostico, calcola_punti_eliminazione, calcola_punti_torneo,
    _safe_int, is_partita_scaduta, FASI_NOMI, PUNTI_KNOCKOUT, PUNTI_TORNEO,
...
def pronostici_eliminazione(fase):
    """Inserimento pronostici per una fase knockout."""
    if 'nome_utente' not in session:
        return redirect(url_for('auth.login'))
    with db_conn() as conn:
        user = _utente_corrente(conn)
        ...
        # Verifica che la fase esista e i pronostici siano sbloccati
        ...
        if request.method == 'POST' and not is_locked:
            for p in partite:
                pid      = row_get(p, 'id')
                vinc     = (request.form.get(f'vincitore_{pid}') or '').strip()
                gc90     = _safe_int(request.form.get(f'gol_casa_{pid}'), lo=0, hi=20)
                go90     = _safe_int(request.form.get(f'gol_ospite_{pid}'), lo=0, hi=20)
                if vinc:
                    if pid in pron_salvati:
                        db_execute(conn,
                                   'UPDATE pronostici_eliminazione SET vincitore=?, gol_casa_90=?, '
                                   'gol_ospite_90=? WHERE id_utente=? AND id_partita=?',
                                   (vinc, gc90, go90, uid, pid))
                    else:
                        db_execute(conn,
                                   'INSERT INTO pronostici_eliminazione '
                                   '(id_utente, id_partita, vincitore, gol_casa_90, gol_ospite_90) '
                                   'VALUES (?,?,?,?,?)',
                                   (uid, pid, vinc, gc90, go90))
            db_commit(conn)
            return redirect(url_for('gioco.bracket'))

        # Se i pronostici sono locked, mostra tutti (anche degli altri)
        tutti_pron = {}
        if is_locked:
            for p in partite:
                pid = row_get(p, 'id')
    ...
    return render_template('pronostici_eliminazione.html',
                           ...
                           partite=partite, pron_salvati=pron_salvati,
                           is_locked=is_locked, tutti_pron=tutti_pron,
                           PUNTI_KNOCKOUT=PUNTI_KNOCKOUT, session=session)
